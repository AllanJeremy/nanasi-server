...
function _getProductTypesByFilter(filter, callback) {
    ...
    return ProductType.find(filter)
        ...
        .then((productTypesFound) => {
            ...
            const message = isOk ?
                FeedbackMessages.itemsFoundWithCount(
                    productTypesFound,
                    "Product types"
                ) :
                FeedbackMessages.itemNotFound("Product types");
...
function _getSingleProductTypeByFilter(filter, callback) {
    return ProductType.findOne(filter)
        ...
        .then((productTypeFound) => {
            ...
            const statusCode = isOk ? 200 : 404;
            const message = isOk ?
                FeedbackMessages.itemsFound("Product type") :
                FeedbackMessages.itemNotFound("Product type");
...
function _getCategoriesByFilter(filter, callback) {
    ...
    return Category.find(filter)
        ...
        .then((categoriesFound) => {
            ...
            const statusCode = isOk ? 200 : 404;
            const message = isOk ?
                FeedbackMessages.itemsFoundWithCount(categoriesFound, "Categories") :
                FeedbackMessages.itemNotFound("Categories");
...
function _getSingleCategoryByFilter(filter, callback) {
    return Category.findOne(filter)
        ...
        .then((categoryFound) => {
            ...
            const statusCode = isOk ? 200 : 404;
            const message = isOk ?
                FeedbackMessages.itemsFound("Category") :
                FeedbackMessages.itemNotFound("Category");
