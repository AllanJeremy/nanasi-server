...
function _getSingleUserByFilter(filter, callback) {
  return User.findOne(filter, (err, userFound) => {
    ...
    return callback(Api.getResponse(isOk, message, userFound, statusCode));
  });
}
  ...
  User.findByIdAndUpdate(userId, updateData)
    .then(updatedUser => {
      return callback(
        Api.getResponse(
          true,
          FeedbackMessages.itemUpdatedSuccessfully("user"),
          updatedUser
        )
      );
    })
  ...
  address
    ...
    .then(addressAdded => {
      ...
      return callback(
        Api.getResponse(
          true,
          FeedbackMessages.operationSucceeded("added address"),
          addressAdded
        )
      );
    })
...
function _getAddressesByFilter(filter, callback) {
  filter = filter || {};
  Address.find(filter,(err, addressesFound) => {
    if (err) {
      return callback(
        Api.getError(
          FeedbackMessages.operationFailed("get user addresses"),
          err
        )
      );
    }

    const addressCount = addressesFound.length;
    const isOk = addressCount > 0;
    const statusCode = isOk ? 200 : 404;
    const message = isOk ? FeedbackMessages.itemsFoundWithCount(addressesFound, "Addresses")
      : FeedbackMessages.itemNotFound("Addresses");
    return callback(
      Api.getResponse(
        isOk,
        message,
...
function _getSingleAddressByFilter(filter, callback) {
  Address.findOne(filter)
    .then(addressFound => {
      if (!addressFound) {
        return callback(
          Api.getResponse(
            false,
            FeedbackMessages.itemNotFound("Address"),
            null,
            404
          )
        );
      ...
      return callback(
        Api.getResponse(
          true,
          FeedbackMessages.itemsFound("Address"),
          addressFound
        )
      );
    })
  ...
  Address.findByIdAndUpdate(addressId, updateData)
    .then(updatedAddress => {
      return callback(
        Api.getResponse(
          true,
          FeedbackMessages.itemUpdatedSuccessfully("address"),
          updatedAddress
        )
      );
    })
...
// Get user addresses 
module.exports.getUserAddresses = (userId, callback) => {
  return _getAddressesByFilter(
    {
      user: userId
    },
    callback
